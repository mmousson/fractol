...
/*   Updated: 2019/02/28 06:17:19 by mmousson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render(float c_r, float c_i, int iterations_max, t_fractol *inf)
{
	int		i;
	float	z_r;
	float	z_i;
	...
	z_r = 0.0f;
	z_i = 0.0f;
	while ((z_r * z_r) + (z_i * z_i) < 4.0f && i < iterations_max)
	{
		tmp = z_r;
	...
	if (i == iterations_max)
		ft_put_pixel(inf->image, inf->x, inf->y, 0xFFFFFF);
}
